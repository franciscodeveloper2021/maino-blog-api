...
RSpec.describe UseCases::UserActions::UpdatePasswordService, type: :service do
  ...
  context "when repository is injected" do
    describe "initialize" do
      it "receives the user repository as a dependency" do
        update_password_service = described_class.new(user_repository)

        expect(update_password_service.instance_variable_get(:@user_repository)).to eq(user_repository)
      end
    end
  end
